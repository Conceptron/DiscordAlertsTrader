...
from DiscordAlertsTrader.alerts_trader import find_last_trade, option_date
...
class AlertsTracker():
    ...
    def trade_alert(self, order, live_alert=True, channel=None):
        ...
        if open_trade is not None and self.lc["portfolio"]:
            # print open trade row
            print("Found open trade")
            print(self.portfolio.loc[open_trade])
        ...
        if order["action"] in ["BTO", "STC",'STO', 'BTC'] and (live_alert):
            if order.get('Actual Cost', 'None') == 'None':
                print("Actual cost missing, getting actual cost for", order["Symbol"], order["action"])
