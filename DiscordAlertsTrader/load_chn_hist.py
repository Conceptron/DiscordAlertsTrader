...
if re_dowload or not op.exists(fname_out):
...
msg_hist.to_csv(f'data/parsed/{author}_parsed.csv', index=False)
...
for ix, row in msg_hist.iterrows():#.loc[ix:].iterrows(): #
    ...
    full_date = order['expDate'] + f'/{year}' if len(order['expDate'].split("/")) == 2 else order['expDate']
    dt_fm = '%m/%d/%y' if len(full_date.split("/")) == 2 else '%m/%d/%Y' if len(full_date.split("/")[2]) == 4 else '%m/%d/%y'
    ...
    if tsm > out.iloc[-1][ 'timestamp']:
        print("Order time outside of market hours",order['action'], order['expDate'], order['Date'], order['Symbol'], ix)
        resp = tracker.trade_alert(order, live_alert=False, channel=author)
        continue
    ...
    bto_date = tracker.portfolio[(tracker.portfolio['Symbol'] == order['Symbol']) & (tracker.portfolio['isOpen'])]
    if len(bto_date):
